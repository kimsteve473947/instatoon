...
export function MiriCanvasStudioFinal() {
  ...
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      // Ctrl/Cmd 키가 눌려있으면 무시 (기본 브라우저 동작 허용)
      if (e.ctrlKey || e.metaKey) {
        return;
      }
      
      // 일반 휠 동작은 스크롤로 처리 (기본 동작)
      // scrollContainerRef가 이미 overflow-y-auto로 설정되어 있어 자동으로 스크롤됨
    };
  ...
  // 배율에 따른 캔버스 간격 계산 (수학적 접근)
  const calculateCanvasGap = useCallback(() => {
    const scale = zoom / 100;
    const MIN_VISUAL_GAP = 40; // 최소 시각적 간격
    const baseGap = 60; // 기본 간격
    
    // 배율이 커질수록 간격도 비례 증가, 최소값 보장
    return Math.max(MIN_VISUAL_GAP, baseGap * scale);
  }, [zoom]);
    ...
    <div className="h-screen w-full flex flex-col bg-slate-50 overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col bg-slate-50 overflow-hidden">
                ...
                >
                  {cuts.map((cut, index) => (
                    <div
                      ...
                      style={{
                        width: CANVAS_SIZES[canvasRatio].width * (zoom / 100),
                        height: CANVAS_SIZES[canvasRatio].height * (zoom / 100)
                      }}
                    >
                      {/* 컷 번호 */}
          ...
          {/* 하단 줌 컨트롤 - 고정 */}
          <div className="h-16 bg-white border-t border-slate-200 flex items-center justify-center px-6 flex-shrink-0">
            <div className="flex items-center gap-6">
              <Button 
                variant="ghost" 
                size="sm" 
